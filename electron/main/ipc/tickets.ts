...
function normalizeIikoReasonValues(value: any): string[] {
  ...
  if (!text || text === '-') return []
  if (meta.iikoReasons[text]) return [text]
...
function getIikoReasons(raw: any): TicketReasonItem[] {
  return normalizeIikoReasonValues(getRawIikoReasonValue(raw)).map(id => ({
    id,
    name: meta.iikoReasons[id] ?? id
  }))
}

function normalizeTicketTagValues(value: any): string[] {
...
function getTicketTags(raw: any): TicketTagItem[] {
  return Array.from(new Set(normalizeTicketTagValues(raw?.tags ?? raw?.tag_list ?? raw?.tagList ?? raw?.ticket_tags ?? raw?.TicketTags))).map(id => ({
    id,
...
function filterTicketsLocally(tickets: any[], cond: Conditions, myUserId: number | null): any[] {
  return tickets.filter(t => {
    ...
    const iikoReasons = getIikoReasons(t).map(reason => reason.id)
    const tags = getTicketTags(t).map(tag => tag.id)
...
function buildZammadQuery(cond: Conditions, myUserId: number | null): string {
  ...
  if (cond.iikoReasons?.length) {
    parts.push(`${meta.iikoReasonField}:(${cond.iikoReasons.map(reason => zammadSearchValue(reason.id)).join(' OR ')})`)
  }
  if (cond.tags?.length) {
